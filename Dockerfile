FROM fluke667/dockbase-nextcloud:latest
...
RUN GPG_KEYS=B0F4253373F8F6F510D42178520A9993A1C052F8 \
  && CONFIG="\
    ...
    --with-file-aio \
    --with-http_v2_module \
    --add-module=/usr/src/ngx_devel_kit-$VER_DEVKIT \
    --add-module=/usr/src/lua-nginx-module-$VER_LUA \
    --add-module=/usr/src/ngx_http_geoip2_module-$VER_GEOIP2 \
  " \
  && addgroup -S nginx \
  ...
  && apk add --no-cache --virtual .build-deps \
    ...
    perl-dev \
    luajit-dev \
  && curl -fSL http://nginx.org/download/nginx-$VER_NGINX.tar.gz -o nginx.tar.gz \
  && curl -fSL http://nginx.org/download/nginx-$VER_NGINX.tar.gz.asc  -o nginx.tar.gz.asc \
  && curl -fSL https://github.com/simpl/ngx_devel_kit/archive/v$VER_DEVKIT.tar.gz -o ndk.tar.gz \
  && curl -fSL https://github.com/openresty/lua-nginx-module/archive/v$VER_LUA.tar.gz -o lua.tar.gz \
  && curl -fSL https://github.com/leev/ngx_http_geoip2_module/archive/$VER_GEOIP2.tar.gz -o ngx_http_geoip2_module.tar.gz \
  ...
  && rm -rf /usr/src/nginx-$VER_NGINX \
  && rm -rf /usr/src/ngx_http_geoip2_module-$VER_GEOIP2 \
  \
